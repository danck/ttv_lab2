...
//			 add potential predecessor to successor list and finger table and
	...
	public final void broadcast(Broadcast info) throws CommunicationException {
		...
		this.logger.info("Send message for boradcast");
		
		List<Node> ft = references.getFingerTableEntries();
		Collections.sort(ft);
		
		for (Node n: ft) {
			// TODO: decrement range according to own range and stuff
			if (???) {
				this.logger.info("Forwarding broadcast to " + info.getTarget());
				Broadcast bc = new Broadcast(
						findSuccessor(n.getNodeID()).getNodeID(), 
						info.getSource(), 
						n.getNodeID(), 
						info.getTransaction(),
						info.getHit()
